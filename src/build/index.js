...
export default function build(source, config = {}, typeDeps = []) {
  ...
  // if there is only one type definition and it's not named Query, build the type and return it
  if (
    definitions.length === 1 &&
    definitions[0].name.value !== 'Query' &&
    Object.keys(buildMap).includes(definitions[0].kind)
  ) {
